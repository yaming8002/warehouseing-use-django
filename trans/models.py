...
class TransportLog(models.Model):
    ...
    construction = models.ForeignKey(Construction, related_name='transport_destination_logs', on_delete=models.CASCADE, verbose_name="目的地")
    whse = models.ForeignKey(WhseList,on_delete=models.CASCADE,verbose_name="倉庫")
    transfer_constn = models.ForeignKey(Construction, related_name='transport_source_logs', on_delete=models.CASCADE, verbose_name="來源地", null=True)
...
class TransportDetailLog(models.Model):
    logistics_id = models.ForeignKey(TransportLog, on_delete=models.CASCADE, verbose_name="運輸")
    material_id = models.CharField(max_length=10, verbose_name="物料")
    quantity = models.IntegerField(default=0, verbose_name="數量")
    all_quantity = models.IntegerField(default=0, verbose_name="總數量")
    unit = models.DecimalField(max_digits=10, decimal_places=2, default=-1 ,verbose_name="單位量")
    all_unit = models.DecimalField(max_digits=10, decimal_places=2, default=-1,verbose_name="總單位量")
    remark = models.TextField(verbose_name="備註")

    class Meta:
        unique_together = ['logistics_id', 'material_id']
