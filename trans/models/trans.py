...
class TransLog(models.Model):
    ...
    carinfo = models.ForeignKey(CarInfo,null=True, on_delete=models.CASCADE, verbose_name="車輛")
...
class TransLogDetail(models.Model):
    ...
    @classmethod
    def create(cls, tran: TransLog, item: list):
        unit_req = item[9]
        # print(type(unit_req))
    ...
    def rollback(cls, tran ):
        detials = cls.objects.select_related('translog').filter(translog__code=tran).all()
