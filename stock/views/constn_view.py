...
from stock.service.steel_component import component_map, build_component_table, mat_tree
...
def component_view(request):
    ...
    if request.method == "POST":
        ...
        get_level_val = request.POST.get("level")
        table_level = int(get_level_val) if get_level_val else table_level
        selected_items_map = [item for item in component_list if str(item['id']) in selected_items]
        print('selected_items_map',selected_items)
...
def tool_view(request):
    ...
    if request.method == "POST":
        ...
        selected_items_map = [item for item in tool_list if  str(item['id']) in selected_items]
