...
CREATE PROCEDURE proc_move_old_by_month(
    IN firstfay DATE,
    IN lastday DATE,
    IN delete_translog BOOLEAN  -- 新增布爾參數，用來控制是否刪除 TransLog 相關資料
)
BEGIN
    DECLARE yyyy INT;
    DECLARE mm INT;

    -- 設置 delete_translog 的默認值
    IF delete_translog IS NULL THEN
        SET delete_translog = TRUE;
    END IF;
    ...
    -- 判斷是否刪除 TransLogDetail 和 TransLog 資料
    IF delete_translog = TRUE THEN
